...
// ✅ CORS (deixa mais flexível para prod/local)
app.use(
  cors({
    origin: (origin, cb) => {
      const allowed = [
...
app.post("/checkout/create-preference", async (req, res) => {
  try {
    ...
    if (!plan || !plan.is_active) return res.status(404).json({ error: "Plano inválido ou inativo" });
    ...
    const payload = {
      ...
      items: [{ title: plan.title || plan.name || `Plano ${plan.id}`, quantity: 1, currency_id: "BRL", unit_price: Number(plan.price) }],
    ...
    const preference = await mpFetch("/checkout/preferences", { method: "POST", body: JSON.stringify(payload) });
