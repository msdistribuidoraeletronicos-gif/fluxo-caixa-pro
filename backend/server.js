...
// ✅ NOVA FUNÇÃO DE CÁLCULO (Solicitada)
...
function daysFromBillingPeriod(billingPeriod) {
...
app.post("/checkout/create-preference", async (req, res) => {
  try {
    ...
    const BACKEND_PUBLIC = (process.env.BACKEND_PUBLIC_URL || "http://localhost:3001").trim();

    let successUrl, failureUrl, pendingUrl, notifyUrl;
    try {
      successUrl = new URL("/?mp=success", FRONTEND).toString();
      failureUrl = new URL("/?mp=failure", FRONTEND).toString();
      pendingUrl = new URL("/?mp=pending", FRONTEND).toString();
      notifyUrl = new URL("/webhook/mercadopago", BACKEND_PUBLIC).toString();
    } catch (err) {
      console.error("Erro ao construir URLs:", err);
      return res.status(500).json({ error: "URLs inválidas no .env" });
    }
    ...
    console.log("MP preference payload:", payload);
...
// 3) WEBHOOK MERCADO PAGO (ATUALIZADO E CORRIGIDO)
app.post("/webhook/mercadopago", async (req, res) => {
  try {
    ...
    const updateData = {
      ...
      // ✅ CAMPOS NOVOS PARA CICLO E ÂNCORA
      ...
      // Limpa trial se houver pagamento
      trial_ends_at: null,
    };

    // Remove campos nulos se não aprovado para não sobrescrever dados existentes errados
    if (!isApproved) {
...
console.log("BACKEND_PUBLIC_URL =", process.env.BACKEND_PUBLIC_URL);
console.log("SUPABASE_URL =", !!process.env.SUPABASE_URL ? "DEFINED" : "MISSING");
console.log("MP_ACCESS_TOKEN =", !!process.env.MP_ACCESS_TOKEN ? "DEFINED" : "MISSING");
console.log("------------------");
